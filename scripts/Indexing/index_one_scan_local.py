...
def get_options(the_parser):
    ...
    the_parser.add_argument(
        "--output-dir",
        help="Specify processing directory",
        dest="output_dir",
        type=str,
        required=True,
    )
    ...
    the_parser.add_argument(
        "-l",
        "--load_step",
        help="Specify load step we are indexing",
        action="store",
        dest="load_step",
        type=str,
        required=True,
...
def main(pars, load_step, load_step_processing_dir, scan_name, key):
    # Work out our sample and grain_volume names
    rawdata_dir = pars.directories.raw_data
    working_dir = os.path.join(load_step_processing_dir, scan_name)
    ...
    if pars.collection_facility == "ESRF":
        ...
        log.info("Merging")
        do_merge(getattr(pars.parameter_files.ImageD11, load_step)[0], peaksearch_flt_path, merged_path, pars.peak_merge.spot_merge_tol, pars.peaksearch.thresholds)
        ...
        if pars.distortion.correct:
            ...
            correct_flt(e2dx, e2dy, merged_path, getattr(pars.parameter_files.ImageD11, load_step)[0], merged_corrected_path)
    ...
    for phase_index in range(pars.phases.total):
        ...
        unindexed_peaks_path = merged_path + ".unindexed"
        ...
        for tol_index, tol in enumerate(pars.makemap.tolseq[phase_index]):
            if tol_index == 0:
                ...
                output_map = output_map_path
            # If we're doing the final makemap, then filter the grains
            if tol_index == len(pars.makemap.tolseq[phase_index]) - 1:
                ...
                makemap(peaks=merged_path,
                        ...
                        filter_grains=True)
                # Then run bootstrap:
                merged_labelled_peaks_path = merged_path + ".new"
...
if __name__ == "__main__":
    ...
    pars = parameter_attribute_builder(options.input_path)
    main(pars, options.load_step, options.output_dir, options.scan_name, options.key)
